...
from api_monitor_toolkit.core.discovery import find_control, find_controls, find_main_window
...
class SpiderController:
    def __init__(self, include_params: bool, include_stack: bool, output: OutputHandler):
    ...
    def run(self):
        ...
        try:
            ...
            tree_hwnd = find_control(main_hwnd, lambda h: win32gui.GetClassName(h) == "SysTreeView32")
    ...
    def _process_summary(self, main_hwnd: int, metadata: dict):
        try:
            panel = find_controls(main_hwnd, lambda h: "Summary" in win32gui.GetWindowText(h))[0]
            header = find_control(panel, lambda h: win32gui.GetClassName(h) == "SysHeader32")
            listview = find_control(panel, lambda h: win32gui.GetClassName(h) == "SysListView32")
        ...
        if not self._wait_for_list_data(listview):
            logger.warning(f"No summary entries loaded for {metadata['filename']} (PID={metadata['pid']})")
            return
            ...
            for i in range(total):
                ...
                if self.include_params:
                    entry["parameters"] = self._get_mapped_panel_data(
                        main_hwnd, "Parameters", PARAMS_MAPPING, skip_penultimate_empty=True
                    )
    ...
    def _wait_for_list_data(self, listview_hwnd: int, retries: int = 5, delay: float = 0.2) -> bool:
    ...
    def _get_mapped_panel_data(
        ...
        try:
            window = find_controls(main_hwnd, lambda h: panel_title in win32gui.GetWindowText(h))[0]
            header = find_control(window, lambda h: win32gui.GetClassName(h) == "SysHeader32")
            listview = find_control(window, lambda h: win32gui.GetClassName(h) == "SysListView32")
                ...
                for i, row in enumerate(data):
                    if skip_penultimate_empty and i == len(data) - 2:
                        if not any(cell.strip() for cell in row.values()):
                            continue
                    mapped.append({
                        (mapped_key := mapping.get(k, k)): self.transformer.transform(mapped_key, v)
